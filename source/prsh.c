...
void prsh_copy_default_bootinfo(boot_info_t *boot_info)
{
    ...
    boot_info->boot_flags = 0xA0000000;
    boot_info->boot_state = 0x84000000;
    ...
    boot_info->boot1_main = 0;
    boot_info->boot1_read = 0;
    boot_info->boot1_verify = 0;
    boot_info->boot1_decrypt = 0;
    boot_info->boot0_main = 0;
    boot_info->boot0_read = 0;
    boot_info->boot0_verify = 0;
}
...
int prsh_exists_decrypted(void)
{
    ...
    while (size)
    {
        if (!memcmp(buffer, "PRSH", sizeof(u32)))
            break;
        buffer += sizeof(u32);
        size -= sizeof(u32);
    }
...
int prsh_is_checksum_valid(prsh_header *header_in)
{
    ...
    if (!header_in || header_in->total_entries > 0x100)
        return 0;
    if (header_in->magic != PRSH_HEADER_MAGIC)
        return 0;
